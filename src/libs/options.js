    ...
    async.series(
        ...
        (err, result) => {
            ...
            if (target.match(/\-/)) {
                ...
                if (net.isIPv4(minHost)) {
                    ...
                    if (net.isIPv4(maxHost)) {
                        ...
                        if (splitTarget[1] >>> 0 === Number(maxHost)) {
                            ...
                            callback(
                                "Invalid IPv4 target range, ie: 192.168.0.1-5, 192.168.0.1-192.168.0.5"
                            )
                            return
                        }
                    }
                    ...
                    callback(
                        "Invalid IPv4 target. ie: 192.168.0.1-5, 192.168.0.1-192.168.0.5"
                    )
                    return
                }

                const numMinHost = splitMinHost.reduce(
                    (prev, val, i) => prev + val * 256 ** (3 - i),
                    0
                )
                const numMaxHost = splitMaxHost.reduce(
                    (prev, val, i) => prev + val * 256 ** (3 - i),
                    0
                )
                ...
                if (!ips) {
                    callback(
                        "Invalid IPv4 target. Please specify a target using --target [cidr|ip|range]"
                    )
                    return
                }
            ...
            if ((targetMatch = target.match(/\//))) {
                ...
                if (!ips) {
                    callback(
                        "Invalid IPv4 CIDR target. Please specify a target using --target [cidr|ip|range]"
                    )
                    return
                }
                callback(null, ips)
...
function help(args) {
    ...
    if (args.about) {
        ...
        console.log(
            "Repository: %s",
            packageInfo.repository.url.replace(/git/, "http")
        )
        process.exit(0)
    }
}
...
function takeCareOfCrazyPeople(args, callback) {
    ...
    if (count > 16580355) {
        let msg =
            "limit of 16580355 numbers of ip/port combinaison reached (" +
            count +
            ")"
        msg +=
            ", see https://github.com/eviltik/evilscan/issues/25 for more information"
        callback(msg)
        return false
    }
    return true
    ...
    async.series(
        ...
        (err, result) => {
            ...
            if (!args.port && !args.reverse && !args.geo) {
                return callback(
                    "Please specify at least one port, --port=80",
                    args
                )
            }
